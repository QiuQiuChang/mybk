<template>
  <div class="playList" ref="myScrollbar" v-loading="loading"
...
export default {
    ...
    getData()
      .then(res => {
        ...
        setTimeout(
          res => {
            console.log(res)
            let data = res.playlist.tracks.slice(0, 100);
            // //调用方法
            this.musicList = filterData(data);
          },
          500,
          res
        );
  ...
  watch: {
    ...
    convertData(now,old){
      let data = now.playlist.tracks.slice(0, 100);
            //调用方法
          this.musicList = filterData(data);
    },
  },
  computed: {
    ...mapGetters(["playMenu", "playNum","barMenu","convertData"]),
